...
import glob
import os
from func_3d.dataset.stots import generate_bbox
...
def load_dataset(path):
    print('开始收集所有图片的框长框宽')
    dataset = []
    three_d_path = os.listdir(path)
    k = 0
    length = len(three_d_path)
    for big_path in three_d_path:
        k += 1
        print('\r数据集收集进度为:{}/{}'.format(k,length),end="")
        big_all_path = os.path.join(path,big_path)
        num_frame = len(os.listdir(big_all_path))
        for i in range(num_frame):
            mask_array = np.array(Image.open(os.path.join(big_all_path, f'{i}.png')))
            mask_array = (mask_array == 255).astype(int)
            if np.sum(mask_array) > 0:
                xmin,xmax,ymin,ymax = get_bbox(mask_array)
                xmin = int(xmin) / 640
                ymin = int(ymin) / 640
                xmax = int(xmax) / 640
                ymax = int(ymax) / 640
                ...
                if xmax == xmin or ymax == ymin:
                    print(os.path.join(big_all_path, f'{i}.png'))
                    continue
...
if __name__ == '__main__':
    ...
    ANNOTATIONS_PATH = "data/new_stots/labels" #xml文件所在文件夹
    CLUSTERS = 3 #聚类数量，anchor数量
    INPUTDIM = 640 #输入网络大小
 
    data = load_dataset(ANNOTATIONS_PATH)
    print('总共有{}张图片'.format(len(data)))

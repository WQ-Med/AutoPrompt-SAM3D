...
class HarDNet(nn.Module):
    def __init__(self, depth_wise=False, arch=85, pretrained=True, weight_path='', out=1, args=None):
        ...
        if pretrained:
            '''
            原代码，是要torch从官网里下载，但是我改为了直接本地下载
            if  not hasattr(torch, 'hub'):

                if arch == 68 and not depth_wise:
                    checkpoint = 'checkpoints/hardnet68-5d684880.pth'
                elif arch == 85 and not depth_wise:
                    checkpoint = 'checkpoints/hardnet85-a28faa00.pth'
                elif arch == 68 and depth_wise:
                    checkpoint = 'checkpoints/hardnet68ds-632474d2.pth'
                else:
                    checkpoint = 'checkpoints/hardnet39ds-0e6c6fa9.pth'

...
def get_network(args, net, use_gpu=True, gpu_device = 0, distribution = True):
    ...
    if net == 'sam2':
        ...
        from sam2_train.build_yolosam import build_yolosam2_video_predictor
        
        sam2_checkpoint = args.sam_ckpt
        model_cfg = args.sam_config

        net = build_yolosam2_video_predictor(config_file=model_cfg, sam2_path=sam2_checkpoint, mode=None)
    else :
...
class EarlyStopping:
    ...
    def __init__(self, patience=14, verbose=False, delta=0):
        """
        Args:
            save_path : 模型保存文件夹
